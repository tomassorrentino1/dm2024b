...
{
  cat( "exp_gc_shutdown()  START\n")
  cat( "pbucket_dir = ", pbucket_dir, "\n" )
  cat( "pwf_dir = ", pwf_dir, "\n" )
  cat( "pnombrewf = ", pnombrewf, "\n" )
...
exp_wf_init <- function( pnombrewf )
{
  ...
  if( !file.exists( "bitacora_run.txt" ) )
  {
    linea <- paste0("fecha\tid\tstatus\texperimento\tfuncion\n")
    cat( linea, file="bitacora_run.txt" )
  }
...
exp_init <- function( pbypass=FALSE )
{
  ...
  output$expenv$myexp <- ""
  output$expenv$funcname <- as.list(sys.call(-1))[[1]]
  output$expenv$bypass <- pbypass
  output$resultado <- 0L

  cat( "exp_init()  env_up2$param_local$lastexp  = ", env_up2$param_local$lastexp, "\n")
  ...
  cat( "env_up2$param_local$instruction = ", env_up2$param_local$instruction  , "\n" )
...
crear_experimento_nuevo <- function( pparam_local )
{
  ...
  cat( "crear_experimento_nuevo() ", pparam_local$expenv$softlink, exp_carpeta, "\n" )
  ...
  {
    cat( "pparam_local$expenv$datasets_dir = ", pparam_local$expenv$datasets_dir, "\n" )
    dataset_abs <- paste0( pparam_local$expenv$datasets_dir, "/", pparam_local$archivo )
    exp_checkfile_abort( dataset_abs, pparam_local$expenv$carpeta_wf, nom_experimento )
    cat( "exp_carpeta : ", exp_carpeta, "\n" )
    setwd( exp_carpeta )
  ...
  if( "input_exps" %in%  names( pparam_local ) )
  {
    ...
    for( vexp in pparam_local$input_exps )
    {
        ...
        )
      {
        ...
        cat( "pparam_local$expenv$bypass =  ", pparam_local$expenv$bypass, "\n" )
        cat( "z-Rcanbypass.txt =  ", file.exists( paste0( pparam_local$expenv$exp_dir, "/", vexp, "/z-Rcanbypass.txt") ), "\n" )
        cat( "Input no usable :",  vexp, "\n" )
...
exp_correr_script <- function( pparam_local )
{
  ...
  cat( "exp_correr_script() busqueda\n" )
  print( busqueda )
  ...
  if( busqueda$encontre & busqueda$Rend )
  {
    ...
    linea <- paste0(  format(Sys.time(), "%Y%m%d %H%M%S"), "\t", # la fecha y hora
      pparam_local$expenv$instruction, "\t",
      "previo_end\t",
      exp_carpeta, "\t",
      pparam_local$expenv$funcname, "\n"
    )
    cat( linea, file="bitacora_run.txt", append=TRUE )
  ...
  if( busqueda$encontre & busqueda$resume & !busqueda$bypass )
  {
    ...
    cat( "paso a la carpeta = ", exp_carpeta, "\n")
  ...
  cat( "exp_correr_script() encontre2\n" )
